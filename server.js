...
app.get('/api/tasks', async (req, res) => {
  try {
    const rows = await allDb('SELECT * FROM tasks ORDER BY createdAt DESC');
    res.json(rows);
  } catch (error) {
...
app.get('/api/stats', async (req, res) => {
  try {
    const rows = await allDb('SELECT status, COUNT(*) AS count FROM tasks GROUP BY status');
...
app.post('/api/tasks', async (req, res) => {
  const { name, duration, status, completedAt } = req.body;
  ...
  try {
    const result = await runDb(
      'INSERT INTO tasks (name, duration, status, completedAt, createdAt) VALUES (?, ?, ?, ?, ?)',
      [name, duration, status, completedAt || null, new Date().toISOString()]
    );
